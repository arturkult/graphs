...
#   ./test.py KLASA MODUŁ
#
# KLASA powinna być klasą zdefiniowaną w module MODUŁ.  Tester sprawdza, czy
# podana klasa prawidłowo reprezentuje grafy proste.
# Test polega na wykonaniu kilku elementarnych operacji na wszystkich parami
# nieizomorficznych grafach o co najwyżej 10
# wierzchołkach i porównaniu rezultatów z wzorcem.  Testowi podlegają
...
# Tester kończy pracę w momencie stwierdzenia pierwszego błędu lub poprawnego
# zaliczenia całości.
try:
    ...
    if [x for x in dir(TestedGraph) if not x.startswith("_")] != [x for x in
    dir(Graph) if not x.startswith("_")]:
    ...
    for g6 in graphs:
        ...
        try:
            #print("Test konwersji z/do formatu g6")
        ...
        try:
            #print("Test dodawania i usuwania wierzchołków")
            g, h = TestedGraph(g6), Graph(g6)
            g.addVertex()
        ...
        try:
            #print("Test dodawania, usuwania i sprawdzania istnienia krawędzi")
            g = TestedGraph(g6)
            #print(str(g))
            for u in range(g.order()):
                v = (u - 2) % g.order()
                if u != v:
                    #print("usuwanie " + str(u) + " " + str(v))
                    g.deleteEdge(u, v)
                    #print("usunięto")
                    if g.isEdge(u,v):
